...
public class HttpHeaderParser {
    ...
    //ANDRODMEDA CUSTOME CHANGES ---

    //5 MINUTES SOFT CACHE
    private static final int SOFT_CACHE_MS = 5 * 60 * 1000;
    //24 MINUTES i.e. ONE DAY HARD CACHE
    private static final int FINAL_CACHE_MS = 24 * 60 * 60 * 1000;
        ...
        if (TextUtils.isEmpty(contentType) || !contentType.contains("image")) {
            ...
            if (!TextUtils.isEmpty(hardCache)) {
                ...
                try {
                    ...
                    softExpireDelay = SOFT_CACHE_MS;
                }
                softExpire = now + softExpireDelay;
            } else {
                softExpire = now + SOFT_CACHE_MS;
            }
            ...
            if (!TextUtils.isEmpty(softCache)) {
                ...
                try {
                    ...
                    hardExpireDelay = FINAL_CACHE_MS;
                }
                finalExpire = now + hardExpireDelay;
            } else {
                finalExpire = now + FINAL_CACHE_MS;
            }

        } else {
            softExpire = Long.MAX_VALUE;
            finalExpire = Long.MAX_VALUE;
        }
